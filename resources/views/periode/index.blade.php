            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition {{ $periode->is_active ? 'bg-green-50/50' : '' }}">
                    <td class="px-6 py-4 font-medium text-slate-800">
                        ...
                        <span class="inline-block mt-1 px-2 py-0.5 rounded text-xs font-bold {{ $periode->semester == 'ganjil' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700' }}">
                            {{ ucfirst($periode->semester) }}
                        </span>
                        <span class="block text-xs text-slate-500 mt-1">Kepala: {{ $periode->nama_kepala_madrasah ?: '-' }}</span>
                    </td>
        ...
        <div class="p-4 hover:bg-slate-50 transition {{ $periode->is_active ? 'bg-green-50' : '' }}">
            <div class="flex justify-between items-start mb-3">
                <div class="flex-1">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold {{ $periode->semester == 'ganjil' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700' }} mt-1">
                        {{ ucfirst($periode->semester) }}
                    </span>
                    <p class="text-xs text-slate-500 mt-1">Kepala: {{ $periode->nama_kepala_madrasah ?: '-' }}</p>
                </div>
                @if($periode->is_active)
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    Aktif
            ...
            <div class="flex gap-2">
                ...
                <form action="{{ route('periode.destroy', $periode->id) }}" method="POST" onsubmit="return confirm('Nonaktifkan periode ini? Data nilai, absensi, rekap, dan riwayat tetap tersimpan.');" class="flex-1">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="w-full px-3 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed" {{ $periode->is_active ? '' : 'disabled' }}>
                        Nonaktifkan
                    </button>
                </form>
            </div>
