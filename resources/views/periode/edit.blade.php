...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
    <form action="{{ route('periode.update', $periode->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nama_periode" value="{{ old('nama_periode', $periode->nama_periode) }}" class="w-full px-4 py-3 rounded-lg border border-slate-300 bg-slate-50 focus:outline-none focus:border-teal-500 focus:ring-teal-500" required>
        </div>
        ...
        <div>
            ...
            <input type="text" name="nama_kepala_madrasah" value="{{ old('nama_kepala_madrasah', $periode->nama_kepala_madrasah) }}" class="w-full px-4 py-3 rounded-lg border border-slate-300 bg-slate-50 focus:outline-none focus:border-teal-500 focus:ring-teal-500" placeholder="Contoh: M. SHOLAHUDDIN MALIKI, S.Pd.I">
        </div>

        <div class="grid grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Tanggal Mulai</label>
                <input type="date" name="start_date" value="{{ old('start_date', $periode->start_date) }}" class="w-full px-4 py-3 rounded-lg border border-slate-300 bg-slate-50 focus:outline-none focus:border-teal-500 focus:ring-teal-500" required>
            </div>
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Tanggal Selesai</label>
                <input type="date" name="end_date" value="{{ old('end_date', $periode->end_date) }}" class="w-full px-4 py-3 rounded-lg border border-slate-300 bg-slate-50 focus:outline-none focus:border-teal-500 focus:ring-teal-500" required>
            </div>
        </div>
