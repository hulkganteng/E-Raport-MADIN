...
<div class="max-w-xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
    <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-6">
        ...
        <div class="border-t border-slate-100 pt-6">
            <h4 class="text-sm font-bold text-slate-800 mb-4">Ganti Password (Opsional)</h4>
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Kosongkan jika tidak ingin mengganti password. Minimal 8 karakter.</p>
                    @error('password')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" class="px-6 py-3 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition font-semibold shadow-lg shadow-teal-500/30">Update User</button>
        </div>
    </form>
</div>
@endsection
