                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex justify-center gap-2">
                            @if($canManageKelas)
                                <a href="{{ route('kelas.manage_mapel', $k) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-indigo-100 text-indigo-700 rounded-lg hover:bg-indigo-200 transition text-xs font-semibold" title="Atur Mapel" aria-label="Atur mapel {{ $k->nama_kelas }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                                    Mapel
                                </a>
                            @endif
                            <a href="{{ route('rekap.index', $k->id) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-teal-100 text-teal-700 rounded-lg hover:bg-teal-200 transition text-xs font-semibold" title="Rekap Nilai" aria-label="Rekap nilai {{ $k->nama_kelas }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                Rekap
                            </a>
                            @if($canManageKelas)
                                <a href="{{ route('kelas.edit', $k) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition text-xs font-semibold" aria-label="Edit {{ $k->nama_kelas }}">
                                ...
                                <form action="{{ route('kelas.destroy', $k) }}" method="POST" onsubmit="return confirm('Yakin hapus kelas ini?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition text-xs font-semibold" aria-label="Hapus {{ $k->nama_kelas }}">
