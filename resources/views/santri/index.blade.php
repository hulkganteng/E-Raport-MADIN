...
@section('content')
@php
    $canManageSantri = auth()->user()->role === 'super_admin';
@endphp
    ...
    <div class="hidden md:block overflow-x-auto">
        <table class="w-full text-left text-sm text-slate-600">
            ...
            <tbody class="divide-y divide-slate-100">
                    ...
                    @if($canManageSantri)
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('santri.edit', $santri) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition text-xs font-semibold" aria-label="Edit {{ $santri->nama_lengkap }}">
                                ...
                                <form action="{{ route('santri.destroy', $santri) }}" method="POST" onsubmit="return confirm('Yakin hapus santri ini?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition text-xs font-semibold" aria-label="Hapus {{ $santri->nama_lengkap }}">
                ...
                @if($santris->isEmpty())
                <tr>
                    <td colspan="{{ $canManageSantri ? 5 : 4 }}" class="px-6 py-8 text-center text-slate-400">Belum ada data santri.</td>
                </tr>
                @endif
            </tbody>
        </table>
