                    ...
                    @if($canManageMapel)
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('mapel.edit', $mapel) }}" class="inline-flex items-center gap-1.5 px-3 py-2 bg-yellow-100 text-yellow-700 rounded-lg hover:bg-yellow-200 transition text-xs font-semibold" aria-label="Edit {{ $mapel->nama_mapel }}">
                                ...
                                <form action="{{ route('mapel.destroy', $mapel) }}" method="POST" onsubmit="return confirm('Yakin hapus mapel ini?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition text-xs font-semibold" aria-label="Hapus {{ $mapel->nama_mapel }}">
